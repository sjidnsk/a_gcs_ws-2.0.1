...
class IrisNpConfig:
    ...
    ellipse_aspect_ratio: float = 1.5
    """椭圆长宽比
    ...
    enable_voronoi_optimization: bool = True
    """启用Voronoi优化（在两批扩张中）

    说明：使用Voronoi图优化种子点分布
    影响：提高区域覆盖的均匀性
    推荐值：True
    """

    enable_voronoi_only_mode: bool = False
    """启用Voronoi优化模式（实验性，仅使用Voronoi）

    说明：仅使用Voronoi优化，不进行常规扩张
    影响：实验性功能，可能影响区域质量
    推荐值：False
    """

    voronoi_max_iterations: int = 10
    """Voronoi优化最大迭代次数
    ...
    voronoi_max_new_seeds: int = 20
    """Voronoi优化最多新增种子点数量

    说明：每次Voronoi优化迭代最多新增的种子点数
    影响：新增种子点越多，覆盖越完整
    推荐值：10-30
    """
    ...
    enable_region_pruning: bool = True
    """启用区域修剪（移除被完全覆盖的冗余区域）
    ...
    pruning_sample_resolution: float = 0.05
    """修剪采样分辨率（米）

    说明：区域修剪时的采样分辨率
    影响：分辨率越高，修剪越精确，但计算时间越长
    推荐值：0.01-0.1
    """
...
def get_high_safety_config() -> IrisNpConfig:
    """获取高安全要求配置
...
def get_fast_processing_config() -> IrisNpConfig:
    """获取快速处理配置
...
def get_balanced_config() -> IrisNpConfig:
    """获取平衡配置
