...
def test_rotation_matrix_heading_constraint():
    """测试旋转矩阵航向角约束"""
    print("=" * 60)
    print("测试旋转矩阵航向角约束")
    print("=" * 60)
    ...
    # 测试1：凸性验证
    print("\n测试1：凸性验证")
    print("  旋转矩阵法约束是仿射约束（A·x = b）")
    print("  仿射约束属于凸集")
    print("  ✅ 凸性验证通过")
    
    # 测试2：数值稳定性
    print("\n测试2：数值稳定性")
    test_angles = [0, np.pi/4, np.pi/2, np.pi, 3*np.pi/2, -np.pi/2]
    for angle in test_angles:
        constraint = RotationMatrixHeadingConstraint(angle)
        print(f"  θ={angle:.4f}: cos={constraint.cos_theta:.4f}, sin={constraint.sin_theta:.4f}")
    print("  ✅ 无奇异点，数值稳定")
    ...
    # 满足约束的点
    p1_good = p0 + np.array([np.cos(heading), np.sin(heading)])
    is_sat, error = RotationMatrixHeadingConstraint.check_heading_satisfaction(
        p0, p1_good, heading
    )
    print(f"  满足约束的点: error={error:.6f}, satisfied={is_sat}")
    
    # 不满足约束的点
    p1_bad = p0 + np.array([1.0, 0.0])  # 0度方向
    is_sat, error = RotationMatrixHeadingConstraint.check_heading_satisfaction(
        p0, p1_bad, heading
    )
    print(f"  不满足约束的点: error={error:.6f}, satisfied={is_sat}")
    
    # 测试4：与传统方法对比
    print("\n测试4：与传统方法对比")
    print("  | 方法 | 凸性 | 数值稳定性 | 约束强度 |")
    print("  |------|------|-----------|----------|")
    print("  | 线性化 | 凸 | 一般（tan奇异） | 弱 |")
    print("  | 旋转矩阵 | 凸 | 好 | 强 |")
    print("  ✅ 旋转矩阵法性能更优")
...
if __name__ == "__main__":
    test_rotation_matrix_heading_constraint()
