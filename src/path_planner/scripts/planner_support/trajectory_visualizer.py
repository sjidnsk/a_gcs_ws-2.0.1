...
class TrajectoryVisualizer:
    ...
    def __init__(self, c_space, config):
        """
        初始化可视化器
    ...
    def _visualize_2d(self, result, original_path: List[Tuple[float, float, float]]):
        ...
        if self.config.save_visualization:
            ...
            output_file = os.path.join(self.config.output_dir, f'corridor_decomposition_{mode}_2d.png')
    ...
    def _visualize_3d_gcs(self, result, original_path: List[Tuple[float, float, float]]):
        ...
        if self.config.save_visualization:
            os.makedirs(self.config.output_dir, exist_ok=True)
            mode = result.iris_mode_used if result.used_iris else 'traditional'
            output_file = os.path.join(self.config.output_dir, f'corridor_decomposition_{mode}_3d.png')
            plt.savefig(output_file, dpi=150, bbox_inches='tight')
    ...
    def _visualize_3d_trajectory_standalone(self, result, original_path: List[Tuple[float, float, float]]):
        ...
        if self.config.save_visualization:
            os.makedirs(self.config.output_dir, exist_ok=True)
            mode = result.iris_mode_used if result.used_iris else 'traditional'
            output_file = os.path.join(self.config.output_dir, f'corridor_decomposition_{mode}_3d_trajectory.png')
