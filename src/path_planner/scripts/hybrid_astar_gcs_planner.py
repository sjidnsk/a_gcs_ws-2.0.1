...
def run_test(scenario: str = 'basic', 
    ...
    Args:
        scenario: 测试场景 ('basic', 'complex', 'narrow')
    ...
    # 创建地图和配置空间
    map_size = 200 if scenario == 'basic' else 250
    obstacle_map = create_test_map(map_size, scenario)
    c_space = SE2ConfigurationSpace(obstacle_map, resolution=0.1)
    
    # 规划路径
    start = (5.0, 5.0, 0.0) if scenario == 'basic' else (3.5, 12.5, 0.0)
    goal = (18.0, 18.0, np.pi/4) if scenario == 'basic' else (21.0, 12.5, 0.0)
    ...
    config = PlannerConfig(
        use_iris=True,
        corridor_width=3.0 if scenario == 'basic' else 1.5,
...
if __name__ == "__main__":
    ...
    if scenario in ['-h', '--help']:
        print("\n使用方法:")
        print("  python hybrid_astar_gcs_planner.py [scenario] [gcs_strategy] [gcs_cost]")
        print("\n参数说明:")
        print("  scenario: 测试场景")
        print("    - basic: 基础场景（默认）")
        print("    - complex: 复杂场景")
        ...
        print("    - smooth: 高平滑性")
        print("\n示例:")
        print("  python3 src/path_planner/scripts/hybrid_astar_gcs_planner.py basic standard lunar_standard")
        print("  python3 src/path_planner/scripts/hybrid_astar_gcs_planner.py complex high_risk lunar_high_risk")
        print("  python3 src/path_planner/scripts/hybrid_astar_gcs_planner.py basic emergency time_optimal")
        sys.exit(0)
